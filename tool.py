...
def jian(infile_name, outfile_name, dictionary):
    ...
    一级简码很直接，固定的25个字母；
    二级简码25*25=625个，常用汉字共1000个，减去25，还有975个，按常用汉字的顺序，赋值二级简码，
    如果码已被占用，给它赋值三级简码，重复的二级简码无意义，你还得再按一个键，要是数字键会更不方便，
    用分号来选第二个候选字还行，不过还是要按三下。

    生成简码的时候，只使用L01.txt，不用常用的设置简码无意义，反面会影响词语的输入。
    """
    # 一级简码
