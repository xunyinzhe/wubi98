...
[小狼毫]自带的配置文件在`程序文件夹/data`，部署后时，用户配置与自带配置合并，生成新的配置文件 `$env:APPDATA/Rime/build`，
...
* `` z` `` 笔画(hspnz)反查五笔，点包含在n里面，例如：`` z`nhhsh `` 查询 `主` 字 
